...
import { dataSources as importedDataSources } from '@/lib/mock-data';

export default function ChatbotOnly() {
  const router = useRouter();
  const [message, setMessage] = useState('');
  const [selectedSources, setSelectedSources] = useState<string[]>([]);
  const [messages, setMessages] = useState<Array<{ role: 'bot' | 'user'; content: string; sources?: boolean }>>([
    { 
      role: 'bot', 
      content: "Hi! I'm Insight Scout, your AI assistant. To get started, select the data sources you'd like me to analyze:",
      sources: true
    }
  ]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-900 relative overflow-hidden flex items-center justify-center p-6">
      ...
      <div className="relative z-10 w-full max-w-3xl">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-3 bg-gradient-to-r from-white via-purple-200 to-blue-200 bg-clip-text text-transparent text-balance">
            Chat with Insight Scout
          </h1>
          <p className="text-slate-200 text-lg">Select your data sources to begin analyzing insights</p>
        </div>
        ...
        <div className="glass-card rounded-3xl overflow-hidden shadow-2xl border-2 border-purple-500/20 glow-purple">
          ...
          <div className="gradient-purple-pink animated-gradient px-8 py-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="font-semibold text-xl text-white">Insight Scout</h2>
                <p className="text-sm text-purple-100">Your AI-powered data assistant</p>
              </div>
            </div>
          </div>
                          ...
                          {dataSources.map(source => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className={`font-medium flex-1 text-left ${isSelected ? 'text-blue-200' : 'text-slate-200'}`}>
                                  {source.name}
                                </span>
                                {isSelected && (
